...
export class AdminService {
  ...
  obtener_config_admin(token:any): Observable<any>{
   ...
   obtener_detalles_orden_admin(id:any,token:any):Observable<any>
   {
    let headers = new HttpHeaders({'Content-Type': 'application/json','Authorization':token});
   ...
   //KPI
   kpi_ganancias_mensuales(token:any):Observable<any>
   {
    let headers = new HttpHeaders({'Content-Type': 'application/json','Authorization':token});

    return this._http.get(this.url + 'kpi_ganancias_mensuales',{headers: headers})
